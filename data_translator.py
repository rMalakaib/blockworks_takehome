...
result_df = pd.DataFrame(columns=['from_key'] + list(unique_proposalIds))

# 3. Add the unique from keys to the dataframe
result_df['from_key'] = unique_from_keys
...
for index, row in tqdm(df.iterrows(), total=len(df), desc="Processing Rows", unit="row", ncols=100):
    ...
    from_key_index = result_df[result_df['from_key'] == row['from_key']].index[0]
    
    # Update the corresponding proposalId column with the solutionChosen value
    result_df.at[from_key_index, row['proposalId']] = row['solutionChosen']
...
result_df.to_csv('outputfile.csv', index=False)
